    ...
    <div className="appt-item">
      ...
      <div className="appt-main">
        ...
        <div className="appt-row2">
          {appt.status === 'done' ? (
            <>
              <span className="appt-service"><Scissors size={13} />{appt.serviceName}</span>
        ...
        <div className="card-list accordion-body">
          ...
          {payments.map(a => (
            <div className="payment-row" key={a.id}>
              <div>
                <p className="payment-service">{a.serviceName}</p>
...
function ReportsPage({ appointments, services }) {
  ...
  const revenueByDay = days.map(d => ({
    ...
    total: appointments.filter(a => a.date === d && a.status === 'done' && (serviceFilter === 'all' || a.serviceName === serviceFilter)).reduce((s, a) => s + a.price, 0),
  }));
  const periodTotal = revenueByDay.reduce((s, d) => s + d.total, 0);
  const doneInRange = appointments.filter(a => a.status === 'done' && days.includes(a.date) && (serviceFilter === 'all' || a.serviceName === serviceFilter));
  ...
  const serviceMap = {};
  scoped.filter(a => a.status !== 'cancelled' && a.serviceName && days.includes(a.date)).forEach(a => { serviceMap[a.serviceName] = (serviceMap[a.serviceName] || 0) + 1; });
...
function BackupPage({ customers, appointments, services, profile, onRestore }) {
  const fileInputRef = useRef(null);
  const [msg, setMsg] = useState(null);

  function handleExport() {
    const payload = { version: 1, exportedAt: new Date().toISOString(), customers, appointments, services, profile };
  ...
  function handleFile(e) {
    ...
    reader.onload = () => {
      try {
        ...
        if (!Array.isArray(data.customers) || !Array.isArray(data.appointments)) throw new Error('bad');
        onRestore(data);
        setMsg({ type: 'ok', text: 'اطلاعات با موفقیت بازیابی شد' });
      } catch (err) {
        setMsg({ type: 'err', text: 'فایل پشتیبان معتبر نیست' });
      }
    };
  ...
  return (
    <div className="page">
      ...
      <div className="report-card">
        <h2>تهیه فایل پشتیبان</h2>
        <p className="about-text">یک فایل شامل همه‌ی نوبت‌ها، مشتری‌ها، خدمات و پروفایل شما ساخته و روی گوشی‌تان ذخیره می‌شود.</p>
        <button className="primary-btn" onClick={handleExport} style={{ marginTop: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
          <DownloadCloud size={17} />دانلود فایل پشتیبان
        </button>
      </div>

      <div className="report-card">
        <h2>بازیابی از فایل پشتیبان</h2>
        <p className="about-text">با انتخاب یک فایل پشتیبان قبلی، اطلاعات فعلی برنامه با آن جایگزین می‌شود.</p>
        <input ref={fileInputRef} type="file" accept="application/json" style={{ display: 'none' }} onChange={handleFile} />
        <button className="primary-btn" onClick={() => fileInputRef.current && fileInputRef.current.click()} style={{ marginTop: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
          <UploadCloud size={17} />انتخاب فایل و بازیابی
        </button>
      </div>

      {msg && <p className="about-text" style={{ marginTop: 4, fontWeight: 700, color: msg.type === 'err' ? '#B84C4C' : 'var(--primary)' }}>{msg.text}</p>}
    </div>
  );
}
...
function CompleteAppointmentModal({ appt, services, onClose, onConfirm }) {
  const [serviceId, setServiceId] = useState(appt.serviceId || services[0]?.id || '');
  ...
  return (
    <Modal title="تکمیل نوبت" onClose={onClose}>
      ...
      <button className="primary-btn" disabled={!serviceId || !price} onClick={() => onConfirm({ serviceId, price: Number(price) })}>ثبت و تکمیل نوبت</button>
    </Modal>
  );
}
...
export default function NovbatamApp() {
  ...
  function saveAppointment(data) {
    ...
    if (data.id) {
      ...
      const a = {
        ...
        serviceId: null, serviceName: '', price: 0,
  ...
  function confirmComplete({ serviceId, price }) {
    const appt = completingAppointment;
    if (!appt) return;
    const service = services.find(s => s.id === serviceId);
    persistAppointments(appointments.map(a => a.id === appt.id ? {
      ...a, status: 'done', serviceId, serviceName: service ? service.name : a.serviceName, price,
    } : a));
  ...
  return (
    <div className="nvbm-root">
      ...
      {tab === 'backup' && <BackupPage customers={customers} appointments={appointments} services={services} profile={profile} onRestore={restoreBackup} />}
      {tab === 'about' && <AboutPage />}
